...
function FeedbackPage({user}) {
  ...
  const [category,setCategory]=useState("Gợi ý tính năng"), [message,setMessage]=useState("");
  const submit=async()=>{if(!message.trim()){setError("Bạn hãy nhập nội dung góp ý trước khi gửi.");return}setSaving(true);setError("");
    try { const response=await fetch("/api/append",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({tab:"Feedback",values:["fb-"+Date.now(),user.userAD,category,message.trim(),"Mới",new Date().toISOString()]})});
      const data=await response.json(); if(!response.ok||!data.ok)throw new Error(data.error||"Không thể lưu góp ý");
      setSent(true);setMessage("");
    } catch (e) { setError(e.message||"Không thể lưu góp ý. Vui lòng thử lại."); } finally { setSaving(false); }
  };
  return <div className="page narrow-page"><div className="page-heading simple"><div><span className="eyebrow">CÙNG PHÁT TRIỂN SẢN PHẨM</span><h1>Góp ý cho Healthier VietinBank</h1><p>Chia sẻ trải nghiệm để ứng dụng hữu ích hơn trong công việc thực tế.</p></div></div><Notice/><div className="two-cards"><section className="panel-card"><h2>Gửi góp ý mới</h2>{sent?<div className="success"><div><Check/></div><h3>Cảm ơn bạn đã chia sẻ!</h3><p>Góp ý đã được lưu vào hệ thống.</p><button className="secondary" onClick={()=>setSent(false)}>Gửi góp ý khác</button></div>:<div className="form-stack"><label>Loại góp ý<select value={category} onChange={e=>setCategory(e.target.value)}><option>Gợi ý tính năng</option><option>Nội dung chưa hữu ích</option><option>Lỗi ứng dụng</option><option>Góp ý khác</option></select></label><label>Bạn muốn chia sẻ điều gì?<textarea value={message} onChange={e=>setMessage(e.target.value)} placeholder="Mô tả tình huống và điều bạn mong đợi..." rows="7"/></label>{error&&<div className="form-error">{error}</div>}<button className="primary" disabled={saving} onClick={submit}>{saving?"Đang lưu…":"Gửi góp ý"}{!saving&&<ArrowRight/>}</button></div>}</section><section className="panel-card"><h2>Góp ý mẫu</h2><div className="feedback-history">{mockFeedback.slice(0,1).map(f=><div><div><Pill tone="cyan">{f.category}</Pill><small>{f.date}</small></div><p>{f.message}</p><span>Trạng thái: <b>{f.status}</b></span></div>)}</div></section></div></div>}
...
export default function App() {
  ...
  return <Shell user={user} page={page} setPage={setPage} onLogout={()=>setUser(null)}>
    ...
    {page==="feedback"&&<FeedbackPage user={user}/>}{page==="account"&&<AccountPage user={user}/>}
